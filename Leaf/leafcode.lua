...
function Leaf:AddSection(title, _)
    ...
    -- Fix: Use TabContent or current tab content as parent
    local targetParent = self.TabContent
    if self.CurrentTab and self.CurrentTab.Content then
        targetParent = self.CurrentTab.Content
    end
    -- Derive a consistent Z-index stack relative to parent
    local parentZ = (targetParent and targetParent.ZIndex) or 2
    local baseZ = math.max(2, parentZ)
    ...
    -- Add missing methods first
    Section.AddTextbox = function(self, text, placeholder, callback)
        local s = self.ButtonStyle or {}
        local Textbox = Instance.new("Frame")
        Textbox.Size = UDim2.new(1, 0, 0, s.Height or 36)
        Textbox.BackgroundColor3 = s.BgColor or Theme.Secondary
        Textbox.BackgroundTransparency = s.InactiveBgTransparency or 0
        Textbox.ZIndex = 2
        Textbox.Parent = Section.Content
        
        local corner = Instance.new("UICorner")
        corner.CornerRadius = UDim.new(0, s.CornerRadius or 8)
        corner.Parent = Textbox
        
        local stroke = Instance.new("UIStroke")
        stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        stroke.Color = s.StrokeColor or Theme.AccentSecondary
        stroke.Thickness = s.StrokeThickness or 1
        stroke.Transparency = s.StrokeTransparencyIdle or 0.88
        stroke.Parent = Textbox
        
        local Label = Instance.new("TextLabel")
        Label.Size = UDim2.new(0.4, -7, 1, 0)
        Label.Position = UDim2.new(0, 14, 0, 0)
        Label.Text = text
        Label.Font = Enum.Font.Gotham
        Label.TextSize = s.TextSize or 14
        ...
        Input.Size = UDim2.new(0.6, -14, 0, 24)
        Input.Position = UDim2.new(0.4, 7, 0.5, -12)
        Input.Text = placeholder or ""
        Input.PlaceholderText = placeholder or "Enter text..."
        Input.Font = Enum.Font.Gotham
        Input.TextSize = s.TextSize or 14
        ...
        Input.FocusLost:Connect(function(enterPressed)
            TweenService:Create(inputStroke, TweenInfo.new(0.2), {Transparency = 0.9}):Play()
            TweenService:Create(stroke, TweenInfo.new(0.2), {Transparency = s.StrokeTransparencyIdle or 0.88}):Play()
            
            if callback and (enterPressed or Input.Text ~= placeholder) then
                callback(Input.Text)
            end
        end)
    ...
    Section.AddLabel = function(self, text, textColor)
        local s = self.ButtonStyle or {}
        ...
        TextLabel.TextSize = s.TextSize or 14
        TextLabel.TextColor3 = textColor or Theme.Text
